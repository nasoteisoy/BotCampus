"""Gold Studio v3 — MATCH Jesus knight character refs (Clash figurine).
Connected overlapping plates, proper silhouettes, black studio Cycles.
"""
...
LOG = Path("/workspace/bot-campus-publish/blender/gold-studio/knights_match_log.txt")
SAMPLES, RES = 272, 1400
...
def reset():
    ...
    try: sc.cycles.adaptive_min_samples = 64
...
def mat(name, rgba, rough=0.3, metal=0.0, emit=0.0, sheen=0.0, spec=0.5):
    ...
    if sheen > 0 and "Sheen Weight" in bsdf.inputs: bsdf.inputs["Sheen Weight"].default_value = sheen
    if emit:
        bsdf.inputs["Emission Color"].default_value = rgba
        bsdf.inputs["Emission Strength"].default_value = emit
    links.new(bsdf.outputs[0], outn.inputs[0]); return m

def M_gold(n="g"): return mat(n, (0.94,0.70,0.18,1), rough=0.28, metal=0.94, spec=0.55)
def M_sil(n="s"):  return mat(n, (0.85,0.87,0.90,1), rough=0.18, metal=0.97, spec=0.8)
def M_gm(n="gm"):  return mat(n, (0.14,0.15,0.17,1), rough=0.30, metal=0.90)
def M_cu(n="cu"):  return mat(n, (0.78,0.42,0.15,1), rough=0.32, metal=0.92)
def M_red(n="r"):  return mat(n, (0.80,0.06,0.04,1), rough=0.38, metal=0.20)
def M_j(n="j"):    return mat(n, (0.10,0.10,0.11,1), rough=0.78, metal=0.0, sheen=0.3)  # DARK joints
def M_fab(n, rgba): return mat(n, rgba, rough=0.80, metal=0.0, sheen=0.55)
def M_em(n, rgba, s=14): return mat(n, rgba, rough=0.4, emit=s)
def M_skin(n="sk"): return mat(n, (0.91,0.72,0.56,1), rough=0.50)
def M_hair(n="hr"): return mat(n, (0.22,0.10,0.04,1), rough=0.60)
...
def bev(o, w=0.04, seg=5):
    ...
    mod.width=w; mod.segments=seg; mod.limit_method="ANGLE"; mod.angle_limit=math.radians(22)
    try: bpy.ops.object.modifier_apply(modifier="B")
    except Exception: pass

def box(name, loc, sc, mtl, b=0.045, rot=(0,0,0)):
    bpy.ops.mesh.primitive_cube_add(size=1, location=loc)
    o = bpy.context.active_object; o.name = name
    o.scale = sc; o.rotation_euler = rot
    bpy.ops.object.transform_apply(scale=True, rotation=True)
    if b: bev(o, b, 5)
    shade(o); o.data.materials.append(mtl); return o

def cyl(name, loc, r, d, mtl, rot=(0,0,0), b=0.018, v=28):
...
def sph(name, loc, r, mtl, segs=26, sx=1, sy=1, sz=1):
    bpy.ops.mesh.primitive_uv_sphere_add(radius=r, location=loc, segments=segs, ring_count=18)
    o = bpy.context.active_object; o.name = name
    if (sx,sy,sz)!=(1,1,1):
        o.scale=(sx,sy,sz); bpy.ops.object.transform_apply(scale=True)
    shade(o); o.data.materials.append(mtl); return o

def cone(name, loc, r1, d, mtl, r2=0.0, rot=(0,0,0), b=0.012):
    bpy.ops.mesh.primitive_cone_add(radius1=r1, radius2=r2, depth=d, location=loc, vertices=20)
    o = bpy.context.active_object; o.name = name
    o.rotation_euler = rot; bpy.ops.object.transform_apply(rotation=True)
    if b: bev(o, b, 2)
    shade(o); o.data.materials.append(mtl); return o
...
def cam_at(sc, loc, tgt, lens=54):
    ...
    c=bpy.context.active_object; c.name="Cam"; sc.camera=c; c.data.lens=lens
...
def frame(sc, pad=1.26, yaw=18, elev=0.08, lens=54):
    ...
    c=(mins+maxs)*0.5; size=maxs-mins
    h=max(size.z,0.1); w=max(size.x,size.y,0.1)
...
def plight(name, loc, e, col=(1,0.5,0.12), sz=0.2):
    ...
    L=bpy.context.active_object; L.name=name
    L.data.energy=e; L.data.color=col; L.data.shadow_soft_size=sz
...
def flame(tag, loc, sc=1.0):
    ...
    x,y,z=loc
    sph(f"{tag}b",(x,y,z),0.13*sc,o,sx=1.15,sy=1.0,sz=0.85)
    sph(f"{tag}m",(x,y,z+0.18*sc),0.10*sc,m,sx=0.9,sy=0.85,sz=1.35)
    cone(f"{tag}u",(x,y,z+0.38*sc),0.09*sc,0.26*sc,m,r2=0.015)
    sph(f"{tag}c",(x,y,z+0.10*sc),0.065*sc,c,sx=0.95,sy=0.9,sz=1.15)
    cone(f"{tag}t",(x,y,z+0.58*sc),0.035*sc,0.20*sc,o,r2=0.002)
    cone(f"{tag}w1",(x-0.05*sc,y,z+0.32*sc),0.04*sc,0.16*sc,o,r2=0.004,rot=(0,math.radians(-20),0))
    cone(f"{tag}w2",(x+0.05*sc,y,z+0.30*sc),0.035*sc,0.14*sc,o,r2=0.004,rot=(0,math.radians(18),0))

# ---- sword upright or angled via tip vector (world placement) ----
def sword(tag, grip, tip_dir=(0,0,1), blen=0.95, gw=0.12, g=None, s=None, gem=False, flbase=False):
    g = g or M_gold(f"{tag}g"); s = s or M_sil(f"{tag}s")
    ...
    if side.length < 0.15: side = up.cross(Vector((1,0,0)))
    side.normalize(); fwd = up.cross(side).normalized()
    def P(t): return (gx+dx*t, gy+dy*t, gz+dz*t)
    def oriented_box(name, center, sx,sy,sz, mtl, bw=0.012):
        bpy.ops.mesh.primitive_cube_add(size=1, location=center)
        o=bpy.context.active_object; o.name=name
        o.matrix_world = Matrix.Translation(center) @ Matrix((side,fwd,up)).transposed().to_4x4() @ Matrix.Diagonal((sx,sy,sz,1))
        bpy.ops.object.transform_apply(location=False, rotation=True, scale=True)
        bev(o, bw, 3); shade(o); o.data.materials.append(mtl); return o

    sph(f"{tag}pom", P(-0.09), 0.05, g)
    # grip as small oriented box
    ...
    if gem:
        ...
        sph(f"{tag}gem", (P(0.10)[0]-fwd.x*0.04, P(0.10)[1]-fwd.y*0.04, P(0.10)[2]-fwd.z*0.04), 0.035, gem_m)
    oriented_box(f"{tag}blade", P(0.12+blen*0.5), gw, 0.035, blen, s, 0.01)
    dark = mat(f"{tag}ful",(0.42,0.45,0.50,1), rough=0.32, metal=0.9)
    oriented_box(f"{tag}ful", (P(0.12+blen*0.5)[0]-fwd.x*0.012, P(0.12+blen*0.5)[1]-fwd.y*0.012, P(0.12+blen*0.5)[2]-fwd.z*0.012),
                 gw*0.35, 0.02, blen*0.8, dark, 0.005)
    # tip as small oriented taper box (avoid stray cones)
...
def build_lava():
    ...
    # connected squat body — OVERLAP parts
    for sx,t in ((-1,"L"),(1,"R")):
        ...
        box(f"shin{t}",(sx*0.22,0.02,0.32),(0.20,0.20,0.32),gm,b=0.035)  # overlaps foot
        box(f"slit{t}",(sx*0.22,-0.11,0.32),(0.05,0.04,0.24),lava,b=0.01)
        box(f"thigh{t}",(sx*0.20,0.0,0.55),(0.20,0.20,0.28),gm,b=0.032)  # overlaps shin

    box("pelvis",(0,0.02,0.72),(0.68,0.40,0.24),gm,b=0.045)  # overlaps thighs
    ...
    # V lava — wide base tapering (stacked)
    ...
    for sx,t in ((-1,"L"),(1,"R")):
        # pauldron overlaps chest
        sph(f"paul{t}",(sx*0.55,0.02,1.32),0.26,gm,sx=1.2,sy=1.0,sz=0.82)
        box(f"pTop{t}",(sx*0.55,0.0,1.50),(0.32,0.22,0.07),cu,b=0.015)
        box(f"pLow{t}",(sx*0.52,0.0,1.16),(0.26,0.20,0.16),gm,b=0.03)
        # arm connected
        ...
        sph(f"hand{t}",(sx*0.64,0.0,0.58),0.11,hand)

    # angular HELMET overlapping neck/chest
    sph("helm",(0,0.04,1.68),0.36,gm,sx=1.08,sy=1.02,sz=0.98)
    ...
    box("jaw",(0,-0.02,1.44),(0.36,0.32,0.18),gm,b=0.035)  # overlaps chest+helm
    for sx,t in ((-1,"L"),(1,"R")):
        cone(f"horn{t}",(sx*0.26,0.10,1.90),0.055,0.20,gm,r2=0.012,rot=(math.radians(-28),0,math.radians(sx*30)))
        # BIG glowing eyes
...
def build_redgold():
    ...
    # CROUCH facing camera (-Y): feet forward, hips back
    for sx,t in ((-1,"L"),(1,"R")):
        box(f"boot{t}",(sx*0.40,-0.28,0.08),(0.20,0.30,0.14),g,b=0.035)
        box(f"toe{t}",(sx*0.40,-0.46,0.07),(0.16,0.14,0.10),g,b=0.025)
        box(f"greave{t}",(sx*0.36,-0.12,0.32),(0.19,0.20,0.36),g,b=0.035,rot=(math.radians(18),0,0))
        sph(f"knee{t}",(sx*0.34,-0.02,0.48),0.11,j)
        # thigh: top toward +Y (hips back), bottom toward knee forward
        box(f"thigh{t}",(sx*0.30,0.08,0.60),(0.19,0.24,0.32),j,b=0.03,rot=(math.radians(-48),0,math.radians(sx*8)))
        for i,off in enumerate((-0.07,0,0.07)):
            cyl(f"tr{t}{i}",(sx*0.30,0.08-off*0.35,0.58+off*0.55),0.105,0.045,j,b=0.006)
        box(f"skirt{t}",(sx*0.26,0.02,0.78),(0.22,0.14,0.26),r,b=0.025,rot=(math.radians(-8),0,math.radians(sx*12)))
    ...
    for sx,t in ((-1,"L"),(1,"R")):
        sph(f"paul{t}",(sx*0.50,0.10,1.48),0.24,r,sx=1.25,sy=1.05,sz=0.85)
        box(f"pT{t}",(sx*0.50,0.08,1.64),(0.30,0.20,0.055),g,b=0.012)
        box(f"pL{t}",(sx*0.48,0.08,1.32),(0.22,0.18,0.14),r,b=0.025)
        # SOLID upper arm connecting pauldron to gauntlet (not floating rings alone)
    ...
    # R arm down + sword (forward-right)
    box("rGaunt",(0.70,-0.05,0.85),(0.17,0.17,0.34),g,b=0.032,rot=(math.radians(35),math.radians(10),math.radians(-15)))
    cyl("rCuff",(0.76,-0.18,0.66),0.15,0.11,g,b=0.016,rot=(math.radians(40),0,0))
    sph("rHand",(0.80,-0.28,0.52),0.095,j)
    sph("rElbow",(0.62,0.05,1.00),0.10,j)

    # L arm raised forward with flame
    box("lGaunt",(-0.55,-0.35,1.28),(0.17,0.17,0.32),g,b=0.032,rot=(math.radians(-50),math.radians(-5),math.radians(10)))
    cyl("lCuff",(-0.58,-0.52,1.45),0.14,0.10,g,b=0.016)
    sph("lHand",(-0.60,-0.62,1.55),0.09,j)
    sph("lElbow",(-0.52,-0.05,1.18),0.10,j)

    # helmet with TWO glowing slits — head slightly tilted
    sph("helm",(0,0.12,1.70),0.28,g,sx=1.02,sy=1.08,sz=1.0)
...
def build_hero():
    ...
    for sx,t in ((-1,"L"),(1,"R")):
        sph(f"paul{t}",(sx*0.46,0.0,1.46),0.21,g,sx=1.3,sy=1.1,sz=0.85)
        box(f"p2{t}",(sx*0.48,0.0,1.28),(0.22,0.18,0.14),g,b=0.022)
        cyl(f"sc{t}",(sx*0.48,-0.16,1.46),0.035,0.035,g,rot=(math.radians(90),0,0))

    # R arm + sword (character right = +X)
    cyl("uaR",(0.52,0.02,1.15),0.10,0.28,d,b=0.012)
    box("foR",(0.56,0.04,0.90),(0.13,0.13,0.28),g,b=0.025)
    sph("haR",(0.56,0.04,0.72),0.08,glove)

    # L arm raised
    cyl("uaL",(-0.48,-0.10,1.22),0.10,0.26,d,b=0.012,rot=(math.radians(-48),0,math.radians(-10)))
    box("foL",(-0.56,-0.32,1.38),(0.13,0.13,0.26),g,b=0.025,rot=(math.radians(-58),math.radians(10),0))
    sph("haL",(-0.60,-0.48,1.55),0.08,glove)

    # FACE — large features
    sph("head",(0,0.02,1.76),0.18,sk,segs=32,sx=1.0,sy=0.95,sz=1.08)
    box("jaw",(0,-0.02,1.62),(0.15,0.14,0.10),sk,b=0.02)
    box("nose",(0,-0.16,1.76),(0.032,0.05,0.045),sk,b=0.006)
    for sx,t in ((-1,"L"),(1,"R")):
        # Large almond amber eyes (Clash/hero style)
        sph(f"ew{t}",(sx*0.065,-0.145,1.79),0.055,ew,segs=16,sx=1.15,sy=0.7,sz=1.0)
        sph(f"ea{t}",(sx*0.065,-0.175,1.79),0.042,ea,segs=14,sx=1.1,sy=0.65,sz=0.95)
        sph(f"pu{t}",(sx*0.065,-0.195,1.79),0.012,pup,segs=8)
        box(f"br{t}",(sx*0.065,-0.15,1.86),(0.06,0.024,0.018),hr,b=0.004)
        sph(f"ear{t}",(sx*0.18,0.0,1.76),0.042,sk,segs=10,sx=0.55,sy=1.0,sz=1.25)
    box("mouth",(0,-0.155,1.64),(0.045,0.014,0.012),mat("lip",(0.75,0.4,0.35,1),rough=0.5),b=0.003)

    # quiff hair
    sph("hb",(0,0.06,1.88),0.19,hr,segs=22,sx=1.1,sy=1.15,sz=0.8)
    sph("qf",(0.02,-0.08,1.98),0.14,hr,segs=18,sx=1.0,sy=1.3,sz=0.75)
    box("qff",(0,-0.14,1.92),(0.16,0.14,0.14),hr,b=0.035)
    for sx,t in ((-1,"L"),(1,"R")):
        sph(f"sh{t}",(sx*0.16,0.02,1.74),0.08,hr,segs=12)

    # cape — exterior burnt orange, interior yellow-orange visible at sides
    ...
    for sx,t in ((-1,"L"),(1,"R")):
        ...
        box(f"cTi{t}",(sx*0.28,0.18,1.30),(0.12,0.06,0.40),ci,b=0.015)  # flash of yellow
...
def build_gold_plate():
    ...
    fab = M_fab("GPf",(0.08,0.09,0.07,1))  # dark olive matte
    ...
    # sun rays as thin radial boxes
    for i,ang in enumerate(range(0,360,45)):
        rad=math.radians(ang)
        box(f"ray{i}",(0.14*math.sin(rad),-0.17,0.92+0.14*math.cos(rad)),(0.028,0.022,0.06),g,b=0.004)
    ...
    for sx,t in ((-1,"L"),(1,"R")):
        sph(f"p1{t}",(sx*0.50,0.0,1.50),0.23,g,sx=1.3,sy=1.1,sz=0.8)
        box(f"p2{t}",(sx*0.52,0.0,1.32),(0.24,0.20,0.14),g,b=0.025)
        box(f"p3{t}",(sx*0.54,0.0,1.20),(0.18,0.16,0.10),g,b=0.018)
    ...
    box("vaR",(0.85,-0.28,1.10),(0.14,0.14,0.30),g,b=0.028,rot=(math.radians(-38),math.radians(-5),math.radians(42)))
    for fi in range(4):
        box(f"fR{fi}",(0.98+(fi-1.5)*0.032,-0.45,0.98),(0.04,0.055,0.10),g,b=0.008)

    # closed helm
    sph("helm",(0,0.02,1.76),0.26,g,sx=1.05,sy=1.0,sz=1.02)
    ...
    for sx,t in ((-1,"L"),(1,"R")):
        box(f"wing{t}",(sx*0.26,0.0,1.76),(0.10,0.14,0.12),g,b=0.016,rot=(0,0,math.radians(sx*-20)))
    ...
    # extra visible flame burst at blade base (in front of guard)
    flame("GPxf", (1.00, -0.62, 1.18), 0.48)
    plight("GPxfL", (1.0, -0.65, 1.2), 90, (1,0.45,0.1), 0.25)

# =====================================================================
BUILDS = [
    ("lava_knight.png", build_lava, dict(pad=1.32, yaw=8, elev=0.05, lens=55)),
    ("redgold_knight.png", build_redgold, dict(pad=1.28, yaw=18, elev=0.10, lens=50)),
    ("hero_knight.png", build_hero, dict(pad=1.22, yaw=12, elev=0.08, lens=52)),
    ("gold_plate_knight.png", build_gold_plate, dict(pad=1.24, yaw=26, elev=0.07, lens=54)),
]

def main():
    if LOG.exists(): LOG.unlink()
    log("=== knights_match v3 START ===")
    for fname, builder, fkw in BUILDS:
        try:
            log(f"--- {fname} ---")
            sc = reset(); builder(); lights(sc, 1.05); frame(sc, **fkw)
