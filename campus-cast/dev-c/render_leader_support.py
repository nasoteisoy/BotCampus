#!/usr/bin/env python3
"""LeaderSupport bible-locked cel chibi renderer. Run: python3 render_leader_support.py"""
...
TEAL = (0x0D, 0x94, 0x88, 255)
TEAL_DK = (0x0A, 0x75, 0x6C, 255)
SILVER = (0xC0, 0xC7, 0xD1, 255)
SILVER_DK = (0x8A, 0x93, 0xA0, 255)
SILVER_LT = (0xE4, 0xE8, 0xED, 255)
SKIN = (0xF5, 0xE6, 0xD3, 255)
HAIR = (0x6B, 0x42, 0x2E, 255)       # warm brown
HAIR_DK = (0x4A, 0x2C, 0x1E, 255)
...
def draw_clipboard_badge(d, cx, cy, s, ow):
    """Tiny silver clipboard pin on chest (badge only, not held)."""
    bw, bh = int(22 * s), int(28 * s)
    x0, y0 = cx - bw // 2, cy - bh // 2
    # board
    rrect(d, [x0, y0, x0 + bw, y0 + bh], int(4 * s), SILVER, max(2, ow - 1))
    # clip at top
    cw, ch = int(12 * s), int(7 * s)
    rrect(d, [cx - cw // 2, y0 - int(3 * s), cx + cw // 2, y0 + ch], int(2 * s), SILVER_DK, max(2, ow - 2))
    # paper lines
    for i in range(3):
        ly = y0 + int(10 * s) + i * int(5 * s)
        d.line([(x0 + int(4 * s), ly), (x0 + bw - int(4 * s), ly)], fill=SILVER_LT, width=max(1, int(2 * s)))
    # pin highlight
    d.ellipse([cx - int(3 * s), y0 + int(2 * s), cx + int(3 * s), y0 + int(8 * s)], fill=SILVER_LT)

def draw_leader_support(pose="idle", scale=1.0):
    ...
    if pose == "working":
        # lean/check work — both arms toward desk ahead
    ...
    ellipse(d, [body_cx - int(50 * s), scarf_cy - int(16 * s), body_cx + int(50 * s), scarf_cy + int(34 * s)], TEAL, ow)
    ellipse(d, [body_cx - int(30 * s), scarf_cy - int(4 * s), body_cx + int(32 * s), scarf_cy + int(18 * s)], TEAL_DK, max(2, ow - 2))
    # scarf tails
    d.polygon(
        [
    ...
    # Silver clipboard badge pin (chest)
    draw_clipboard_badge(d, body_cx + int(26 * s), scarf_cy + int(10 * s), s, ow)

    # --- Short visible neck (QC nit from Leader pass) ---
    # Thick stubby neck peeking between chin and scarf (~12–16px)
    neck_w = int(42 * s)
    ...
    # Warm brown short messy-neat hair
    d.pieslice(
    ...
    # Mouth — small thoughtful smile (supportive, not boss-confident)
    mx = head_cx + face_ox
    my = head_cy + int(36 * s)
    if pose == "alert":
        ellipse(d, [mx - int(9 * s), my - int(5 * s), mx + int(9 * s), my + int(11 * s)], (0x4A, 0x30, 0x30, 255), max(2, int(3 * s)))
    elif pose == "working":
        # concentrated slight smile
        d.arc([mx - int(12 * s), my - int(4 * s), mx + int(12 * s), my + int(10 * s)], 25, 145, fill=OUTLINE, width=max(2, int(3 * s)))
    elif pose == "wave":
        d.arc([mx - int(20 * s), my - int(10 * s), mx + int(20 * s), my + int(16 * s)], 20, 160, fill=OUTLINE, width=max(3, int(4 * s)))
    else:
        # thoughtful small smile
        d.arc([mx - int(16 * s), my - int(8 * s), mx + int(16 * s), my + int(12 * s)], 30, 150, fill=OUTLINE, width=max(2, int(3 * s)))
...
if __name__ == "__main__":
    specs = [
        ("leader_support_hero_ref.png", "idle", 1.0),
        ("leader_support_desk_idle.png", "idle", 1.0),
        ("leader_support_desk_working.png", "working", 1.0),
        ("leader_support_ready_review.png", "wave", 1.0),
        ("leader_support_needs_permission.png", "alert", 1.0),
        ("leader_support_intern_idle.png", "idle", 0.70),
    ]
    frames = []
    for name, pose, sc in specs:
        im = draw_leader_support(pose, sc)
