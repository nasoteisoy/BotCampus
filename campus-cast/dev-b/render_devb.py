#!/usr/bin/env python3
"""Dev B (aliases LiveOps / Dev 3) bible-locked cel chibi renderer. Run: python3 render_devb.py

Neck join: chin→scarf stays closed on working lean (head locked to body_cx with
fixed 3/4 offset; trapezoid neck embeds into scarf) — same nit fix as Dev 2 / SpriteForgeSupport.
"""
...
ROSE = (0xE1, 0x1D, 0x48, 255)
ROSE_DK = (0xBE, 0x12, 0x3C, 255)
ROSE_LT = (0xFB, 0x71, 0x85, 255)
CYAN = (0x22, 0xD3, 0xEE, 255)
CYAN_DK = (0x06, 0xB6, 0xD4, 255)
CYAN_LT = (0x67, 0xE8, 0xF9, 255)
SKIN = (0xF5, 0xE6, 0xD3, 255)
HAIR = (0x14, 0x14, 0x18, 255)       # black short neat
HAIR_DK = (0x08, 0x08, 0x0A, 255)
HAIR_LT = (0x2A, 0x2A, 0x32, 255)
...
def draw_pulse_pin(d, cx, cy, s, ow):
    """Thick cyan pulse/dot chest badge — signal-aware, phone-map readable. Badge only (unarmed)."""
    r = int(22 * s)
    ellipse(d, [cx - r, cy - r, cx + r, cy + r], CYAN, max(3, ow))
    # Inner field
    r_mid = int(16 * s)
    ellipse(d, [cx - r_mid, cy - r_mid, cx + r_mid, cy + r_mid], CYAN_LT, max(2, ow - 2))

    # Pulse ring (concentric signal)
    ring_r = int(11 * s)
    d.ellipse(
    ...
    # Core pulse dot
    hub = int(6 * s)
    ellipse(d, [cx - hub, cy - hub, cx + hub, cy + hub], CYAN_DK, max(2, ow - 2))
    # white highlight on core
    hs = max(2, int(3 * s))
    d.ellipse([cx - hs, cy - hub + int(1 * s), cx, cy - hub + int(1 * s) + hs], fill=WHITE)

    # Tiny pulse blip (signal spike) at top-right of badge for readability
    blip = int(4 * s)
    bx = cx + int(10 * s)
    by = cy - int(10 * s)
    ellipse(d, [bx - blip, by - blip, bx + blip, by + blip], WHITE, max(1, ow - 3))
    ...
    hs2 = max(3, int(5 * s))
    d.ellipse(
        [cx - int(10 * s), cy - int(12 * s), cx - int(10 * s) + hs2, cy - int(12 * s) + hs2],
        fill=WHITE,
    )


def draw_devb(pose="idle", scale=1.0):
    ...
    if pose == "working":
        # lean / ops work — both arms toward desk ahead (empty hands)
    ...
    # Cyan pulse pin (chest badge only)
    draw_pulse_pin(d, body_cx + int(26 * s), scarf_cy + int(12 * s), s, ow)
    ...
    # Black short neat hair — ops/runtime tidy cut
    d.pieslice(
    ...
    # short neat fringe (calmer / cleaner than builder)
    d.polygon(
        [
            (head_cx - int(48 * s), head_cy - hr + int(22 * s)),
            (head_cx - int(36 * s), head_cy - int(4 * s)),
            (head_cx - int(22 * s), head_cy - hr + int(12 * s)),
            (head_cx - int(8 * s), head_cy - int(6 * s)),
            (head_cx + int(6 * s), head_cy - hr + int(10 * s)),
            (head_cx + int(18 * s), head_cy - int(4 * s)),
            (head_cx + int(32 * s), head_cy - hr + int(14 * s)),
            (head_cx + int(44 * s), head_cy - int(0 * s)),
            (head_cx + hr - int(6 * s), head_cy - hr + int(26 * s)),
    ...
    # soft crown highlight (black, subtle)
    ellipse(
        d,
        [
            head_cx - int(16 * s),
    ...
    d.polygon(
        [
            (head_cx - int(8 * s), head_cy - hr + int(16 * s)),
            (head_cx + int(4 * s), head_cy - int(0 * s)),
    ...
    # Eyes — calm focused (ops/runtime, steady)
    ...
    for side in (-1, 1):
        ...
        # calm level brows (steady, not energetic)
        brow = int(-12 * s) if pose == "alert" else int(-4 * s)
        d.arc(
    ...
    if pose == "alert":
        ...
        # focused small confident smile
        d.arc(
            [mx - int(12 * s), my - int(2 * s), mx + int(12 * s), my + int(10 * s)],
            25,
            145,
            fill=OUTLINE,
            width=max(2, int(3 * s)),
        ...
        # confident ready smile (not big energetic grin)
        d.arc(
...
if __name__ == "__main__":
    specs = [
        ("devb_hero_ref.png", "idle", 1.0),
        ("devb_desk_idle.png", "idle", 1.0),
        ("devb_desk_working.png", "working", 1.0),
        ("devb_ready_review.png", "wave", 1.0),
        ("devb_needs_permission.png", "alert", 1.0),
        ("devb_intern_idle.png", "idle", 0.70),
    ]
    frames = []
    for name, pose, sc in specs:
        im = draw_devb(pose, sc)
